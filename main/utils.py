...
# 论文级别：根据作者检索论文
def paper_only_author(db_object, authors, k=30):
    """
    输入：数据库，查询中的作者名list（或），top_K，阈值
    输出：全文检索后的df
    """

    table = db_object.get_table("table_paper")

    res = table.output(["paper_uuid", "title", "author", "file_path"]
                    ).match_text("author", " ".join(authors), k
                    ).to_df()

    return res

# 论文级别：根据内容检索论文
def paper_only_content(db_object, model, contents, k=30, threshold=0.3):
    """
    输入：数据库，向量模型，查询中的内容list，top_K，阈值。
    输出：内容混合检索后的df。混合检索单独是30个，融合后是20个。
    """
    content = " ".join(contents)
    table = db_object.get_table("table_chunk")
    content_embed = model.encode_text(content, task='retrieval.query')
    chunk_res = table.output(["paper_uuid", "chunk_uuid", "chunk_content", "_score"]
                    ).match_text("chunk_content", " ".join(content), k
                    ).match_dense("chunk_embed", content_embed, "float", "cosine", k, {"threshold": str(threshold)}
                    ).fusion("weighted_sum", 20, {"weights": "0.8,1.2"}
                    ).to_df()
    # 转向论文，根据paper_uuid定位到论文表中的记录
    paper_uuids = chunk_res["paper_uuid"].drop_duplicates().tolist()
...
# 论文级别：根据作者和内容检索论文
def paper_author_content(db_object, model, authors, contents, k=30, threshold=0.3):
    """
    输入：查询中的作者list，内容list，top_K，阈值。
    输出：检索后的df
    """
    
    author_res = paper_only_author(db_object, authors,k=10)
    content_res = paper_only_content(db_object, model, contents, k=k, threshold=threshold)

    # 取交集并返回 author_res 的字段
    res = pd.merge(author_res, content_res, on="paper_uuid", how="inner")[["paper_uuid", "title", "author", "file_path"]]
