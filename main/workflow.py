...
def process_single_query_one(query, model, db_object):
    ...
    if judge_pc["is_paper_query"]:
        ...
        if rewrite: # 改写成功，执行混合检索
            ...
            image_res = pd.concat([image_res_1,image_res]) # 把检索到的图放最前面
        else:
            logging.info(f"改写失败，使用原始查询执行chunk混合检索和以文搜图!")
            res,image_res,table_res = chunk_hybid_search(db_object, model, query) # df
            image_res_1 = text_to_image(db_object, query, model) # 以文搜图，一张
            image_res = pd.concat([image_res_1,image_res]) # 合并
...
if __name__ == "__main__":
    ...
    # 自定义日志文件路径和文件名
    file = "test_1"
    log_file_path = os.path.join("RAG/run/log", f'{file}.log')
    # 配置日志
    ...
    logging.info(f"加载向量模型，重排模型，数据库成功！")



    # 查询测试，论文单一查询，论文独立子问题查询，内容多跳查询，内容单一查询
    query = "找一篇论文，内容是用UMAP降维，HDBSCAN聚类，最后做主题分析的。"
    logging.info(f"接收查询：{query}")

    # 执行RAG流程，不包含判断是否进入rag
    flag,res,sub_queries,image_res,table_res = retrieve(query, model, db_object,file)
    
    # 执行生成流程
    answer = generate(query,flag,res,sub_queries,rerank_model,db_object,image_res,table_res)
    print("回答：")
    print(answer)
